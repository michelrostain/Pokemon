    ...
    def ajouter_pokemon(self):
        ...
        for i, p in enumerate(liste_brute):
            ...
            nouveau_p = Pokemon(
                ...
                image_url=p.get("image", "Assets/Images/PokeDefaut.png"), # adapter selon ton JSON
                type_principal=p["types"][0], # On prend le premier type de la liste
                evolution_nom=p["evolution"]
            )

            try:
                img = pygame.image.load(chemin_image).convert_alpha() # .convert_alpha() gère la transparence
